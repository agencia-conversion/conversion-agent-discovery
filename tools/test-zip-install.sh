...
NETWORK="wp-agentic-zip-${RUN_ID}"
DB_CONTAINER="wp-agentic-zip-db-${RUN_ID}"
WEB_CONTAINER="wp-agentic-zip-web-${RUN_ID}"
WP_VOLUME="wp-agentic-zip-wp-${RUN_ID}"
DB_VOLUME="wp-agentic-zip-db-${RUN_ID}"
PORT="${WP_AGENTIC_ZIP_TEST_PORT:-8090}"
...
unzip -q "${ROOT_DIR}/dist/wp-agentic.zip" -d "${TMP_DIR}"
...
docker run -d \
  ...
  -v "${TMP_DIR}/wp-agentic:/var/www/html/wp-content/plugins/wp-agentic:ro" \
...
wp_cli wp core install \
  ...
  --title='WP Agentic ZIP Test' \
  --admin_user=admin \
  --admin_password=admin \
  --admin_email=admin@example.com \
  --skip-email

wp_cli wp plugin activate wp-agentic
wp_cli wp rewrite structure '/%postname%/' --hard

curl -fsS "http://localhost:${PORT}/" >/dev/null
curl -fsS -H 'Accept: text/markdown' "http://localhost:${PORT}/" | grep -q '^# WP Agentic ZIP Test'
